function [pop,ind] = IBM_dd_vbCAL(n0,beta1,beta2,K,mortF,p,period,trsz,effk,pfm,gns,stoc)
%IBM_DD_VBCAL Individual-Based Model for Mustelus spp.
%
% IBM integrating density-dependence, and where growth parameters are
% missing
%   Inputs:
%   n0              starting population abundance
%   beta1,beta2     parameters to compute the asymptotic lengh following
...
for sm=1:simend
    ...
    ind=[ind;recruits];
    % density dependence
    Ca = floor((sm-ind.bd)/12);
    [Na,La,grps]=grpstats(ind.len,{ind.sex,Ca},["numel","mean","gname"]);
    DDFa = Na.*(La.^2);
    DDFa(isinf(DDFa)) = 0;
    DDFs = grpstats(DDFa,grps(:,1),"sum");
    Linf = beta1./(1+beta2*DDFs);
    if (sum(size(Linf) == 0) > 0) || (sum(size(Linf) == 1) == 2)
        Linf = p(:,1);
    end
    p(:,1) = [Linf(1);Linf(2)];
    % yearly summary
